...
class ChatbotService {
  ...
  async analyzeUserIntent(message) {
    try {
      const prompt = `
        ...
        - Category có thể là: "Áo nam", "Áo nữ", "Quần nam", "Quần nữ", "Phụ kiện", "Giày dép"
  ...
  async searchProducts(searchParams, limit = 10) {
    try {
      let query = {};
      let sort = {};
  ...
  async generateResponse(userMessage, searchResults, searchParams) {
    try {
  ...
  async processChat(userMessage) {
    try {
      ...
      if (analysis.intent === "search_product") {
        searchResults = await this.searchProducts(analysis.searchParams);
      }

      // 3. Tạo response thông minh
      const response = await this.generateResponse(userMessage, searchResults, analysis.searchParams);
