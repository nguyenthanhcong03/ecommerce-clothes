...
const login = catchAsync(async (req, res) => {
  ...
  if (user.isBlocked) throw new ApiError(403, "Tài khoản đã bị khóa");

  // Verify password
  const isPasswordValid = await bcrypt.compare(password, user.password);
  if (!isPasswordValid) throw new ApiError(401, "Email hoặc mật khẩu không chính xác");
...
const changePassword = catchAsync(async (req, res) => {
  ...
  if (!isOldPasswordValid) throw new ApiError(400, "Mật khẩu cũ không đúng");
...
const refreshToken = catchAsync(async (req, res) => {
  ...
  if (!user) throw new ApiError(401, "Token không hợp lệ");
