...
const Button = ({
  ...
  const getButtonClasses = () => {
    let baseClasses =
      'px-2 py-1 sm:px-2 sm:py-2 md:px-3 md:py-2 md:text-[13px] lg:px-4 lg:py-2 font-[350] focus:outline-none transition-colors duration-200 flex items-center justify-center border border-primaryColor';

    // Kích thước (size)
    switch (size) {
      case 'small':
        baseClasses =
          'px-2 py-1 text-sm rounded-[2px] font-[350] focus:outline-none transition-colors duration-200 flex items-center justify-center';
        break;
      case 'medium':
        baseClasses =
    ...
    switch (variant) {
      case 'primary':
        ...
        break;
      case 'secondary':
        baseClasses += ' bg-white text-text1 hover:bg-gray-100 border border-primaryColor';
        break;
      case 'error':
