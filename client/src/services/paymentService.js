...
/**
 * Tạo URL thanh toán VNPay để chuyển hướng người dùng đến cổng thanh toán VNPay
...
export const createVnpayPayment = async (payload) => {
  ...
  try {
    const response = await instance.post(`${BASE_API}/vnpay/create`, payload);
...
export const createMomoPayment = async (payload) => {
  try {
    const response = await instance.post(`${BASE_API}/momo/create`, payload);
    return response;
  } catch (error) {
