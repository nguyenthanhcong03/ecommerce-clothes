...
const OrderProductItem = memo(({ product }) => {
  const price = product.snapshot?.price || product.snapshot?.originalPrice || 0;
          ...
          <div className='mt-8'>
            ...
            <div className='rounded-lg bg-gray-50 p-4'>
              <div className='flex flex-col sm:flex-row sm:items-start sm:justify-between'>
                <div>
                  <p className='text-base font-medium text-gray-900'>{order.data.shippingAddress?.fullName || 'N/A'}</p>
                  <p className='mt-2 text-sm text-gray-500'>
                    {order.data.shippingAddress?.street || 'N/A'}
                    <br />
                    {order.data.shippingAddress?.ward.name}, {order.data.shippingAddress?.district.name},{' '}
                    {order.data.shippingAddress?.province.name}
                  </p>
                  <p className='mt-2 text-sm text-gray-500'>
                    <span className='font-medium'>Điện thoại:</span> {order.data.shippingAddress?.phoneNumber || 'N/A'}
                  </p>
                  <p className='text-sm text-gray-500'>
                    <span className='font-medium'>Email:</span> {order.data.shippingAddress?.email || 'N/A'}
                  </p>
                </div>
              ...
              <div className='overflow-hidden rounded-lg border bg-white'>
                ...
                <div className='bg-gray-50 p-4'>
                  <div className='flex justify-between text-base font-medium text-gray-900'>
                    <p>Tạm tính</p>
                    <p>{formatCurrency(order.data.totalPrice - order.data.shippingFee)}</p>
                  </div>
                  ...
                  <div className='flex justify-between py-1 text-base font-medium text-gray-900'>
                    <p>Phí vận chuyển</p>
                    <p>{formatCurrency(order.data.shippingFee || 0)}</p>
                  </div>
